...
Usage:
    ...
    python ax_native_benchmark.py --mode soo --trials 50 --replications 10

    # Run MOO benchmark
    python ax_native_benchmark.py --mode moo --trials 50 --replications 10

    # Run integration test (vanilla ax vs W&B e2e)
    python ax_native_benchmark.py --mode integration --trials 20 --replications 5
"""
...
def benchmark_method(
    ...
    results = []
    config = problem.create_sweep_config("ax")  # Use ax config format
    ...
    for i in range(num_replications):
        ...
        print(f"    Rep {i+1}/{num_replications} (seed={seed}): {metric}={result.final_value:.6f}")
...
def run_soo_benchmark(problems: list[str], num_trials: int, num_replications: int, seed: int):
    ...
    for problem_name in problems:
        ...
        for method_name, node_factory in [
            ...
            print(f"  {method_name}:")
            results = benchmark_method(problem, node_factory, num_trials, num_replications, seed)
